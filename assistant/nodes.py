...
def determine_document_search(state: GraphState):
    ...
    response = determine_document_search_chain(query=query, chat_history=chat_history)
...
def answer_question(state: GraphState):
    ...
    response = answer_question_chain(query, chat_history, document_store_needed, context_documents)
